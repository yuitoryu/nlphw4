...
def create_augmented_dataloader(args, dataset):
    ...
    # Randomly choose 5000 samples for augmentation
    num_aug = 5000
    random.seed(42)
    aug_indices = set(random.sample(range(len(train_dataset)), num_aug))
    print(f"Selected {len(aug_indices)} samples for augmentation")

    # Apply transformation only to those samples
    def conditional_transform(example, idx):
        if idx in aug_indices:
            return custom_transform(example)
        return example
    ...
    # Tokenize using the same tokenize_function as the rest of main.py
    ...
    # Create DataLoader consistent with the rest of the code
    ...
    print(f"Augmented DataLoader ready: {len(train_dataset)} total samples "
          f"({num_aug} transformed)")
...
if __name__ == "__main__":
    ...
    # dataset = load_dataset("imdb", revision="c90d9607c0b6d4e1ce6f9641f9dbef0b11d7a5f9")
    ...
    if args.debug_train:
        train_dataloader = DataLoader(small_train_dataset, shuffle=True, batch_size=args.batch_size, num_workers=6)
        ...
        train_dataloader = DataLoader(tokenized_dataset["train"], shuffle=True, batch_size=args.batch_size, num_workers=6)
